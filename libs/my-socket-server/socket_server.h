...
public:
	...
	static getInstance();
	// create a udp socket handle, attach opaque with it. udp socket don't need call socket_server_start to recv message.
	// if port != 0, bind the socket. if addr == NULL, bind ipv4 0.0.0.0. If you want to use ipv6, addr can be "::" and port 0.
	static socketServer* createSocketServerUdp(const char *addr, int port);
