...
/////////////////////////////////////////UDP/////////////////////////////////////////
socketServer* socketServer::createSocketServerUdp(const char* addr, int port)
{
	int fd;
	int family;
	if (addr != NULL || port != 0) {
...
int socketServer::doBind(const char *host, int port, int protocol, int *family) {
	printf("doBind host = %s port = %d protocol = %d\n", host, port, protocol);
	int fd;
	int status;
	int reuse = 1;
	struct addrinfo ai_hints;
